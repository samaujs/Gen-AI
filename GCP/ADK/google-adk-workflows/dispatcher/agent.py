...
weather_tool = agent_tool.AgentTool(agent=weather_agent)
...
root_agent = LlmAgent(
   ...
   - Use the WeatherAgent to check current weather details at the destination

   Based on the user request, sequentially invoke the sub-agents and tools to gather all necessary trip details:
   - Flight details (from FlightAgent)
   - Hotel booking confirmations (from HotelAgent)
   - Sightseeing information (from SightSeeingAgent)
   - Weather conditions (from WeatherAgent)
   ...
   """,
    tools=[flight_tool, hotel_tool, sightseeing_tool, weather_tool]
)
